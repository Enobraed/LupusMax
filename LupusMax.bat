...
set "launcher_url=https://raw.githubusercontent.com/Enobraed/LupusMax/main/LupusMax.bat"
set "local_launcher=%base%LupusMax.bat"
set "temp_launcher=%base%LupusMaxTemp.bat"

:: Проверка наличия старого файла
if exist "%temp_launcher%" del /f /q "%temp_launcher%"

:: Скачивание последней версии скрипта
curl -L -o "%temp_launcher%" "%launcher_url" >nul 2>&1

:: Проверка, был ли скачан файл
if not exist "%temp_launcher%" (
    echo Ошибка загрузки обновленного скрипта. Проверьте подключение к интернету.
    exit /b
)

:: Заменяем старую версию скрипта на новую
copy /Y "%temp_launcher%" "%local_launcher%" >nul 2>&1
del /f /q "%temp_launcher%" >nul 2>&1

:: Перезапуск, чтобы применять обновления
goto :restart
